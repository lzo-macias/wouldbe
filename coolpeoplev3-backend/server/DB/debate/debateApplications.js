...
const validateStream = (stream) => {
    ...
    // Accepts a bare 'YYYY-MM-DD' (what a <input type="date"> gives) as well as
    // a full instant. A date-only string parses as midnight UTC, so it is nudged
    // to midday to keep the calendar day the same in every western timezone —
    // otherwise a US sponsor picking the 19th sees the 18th on their own screen.
    const raw = stream.scheduled_at;
    if (!raw) throw httpError(400, "the debate needs a start date");
    const dateOnly = typeof raw === "string" && /^\d{4}-\d{2}-\d{2}$/.test(raw.trim());
    const when = new Date(dateOnly ? `${raw.trim()}T12:00:00Z` : raw);
    if (Number.isNaN(when.getTime())) {
        throw httpError(400, "the start date is not a valid date");
    }
    // Compared by DAY, not by instant: a date picked as "today" is midday today,
    // which is behind Date.now() by the afternoon and would reject a valid choice.
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    if (when.getTime() < today.getTime()) {
        throw httpError(400, "the start date is in the past");
    }
...
const submitDebateApplication = async ({
    ...
    const streamDay = broadcast.scheduled_at.slice(0, 10);
