...
const isPledgeEligible = async ({ userId, wouldbeId, amount_cents }) => {
    ...
    const wb = await client.query(`SELECT user_id, launch_status, retired FROM wouldbe WHERE id = $1`, [wouldbeId]);
    if (!wb.rows.length) throw httpError(404, "WouldBe not found");
    // SELF-PLEDGE. A candidate backing their own campaign would inflate the
    // pledge total and the backer count with money that was never a third-party
    // commitment — the two numbers the goal, the ranking and the all-or-nothing
    // gate are all read from. It has to be refused HERE and not only in the UI:
    // the route is a plain authenticated POST, so a hidden button stops nobody.
    if (wb.rows[0].user_id === userId) {
        return { eligible: false, reason: "you cannot pledge to your own campaign" };
    }
